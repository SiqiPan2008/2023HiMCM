...
\begin{abstract}
	...
	We seldom mention that dandelion, officially named \textit{Taraxacum officinale}, is know as an invasive species, which is likely to cause economic or environmental harm or harm to human health.
	...
	We input 3 groups of random variables to our model, i.e., temperature, wind and humidity.  Some details like equivalent temperature curve, dispersal distance under horizontal wind and updrafts, random biological characteristics/behaviors are also taken into account.
	
	We output all the locations of dandelions that will spread on the land over time.  In order to get stable results, we run the model with 2 phases and for each phase we run 1000 times.  In the first phase, we determine the optimal starting time and in the second phase, we fix the starting time and get the most probable outcome.
	
	We apply our model to 6 locations and conclude the statistic characteristics.  We also do the sensitivity analysis and discuss the strength and weakness.
	
	\vspace{0.5cm}
	
	In this paper, we also devise a \textbf{Plant Impact Factor Model} to measure the impact of invasive plants on humans and the environment.  We define a global impact factor and a local impact factor for different purpose.
	...
	We apply our model to 226 invasive species and calculate their global impact factors.  We also chose 5 plants and calculate their local impact factors at 3 locations.
	
	We do sensitivity analysis and discuss the strength and weakness.
	
	\vspace{0.5cm}
	At the end of this paper, we write an article to elaborate our viewpoint to invasive species.  Rather think they do the harm to our planet, we need to learn how to get along with them.  To we human beings, they are just vigorous friends.
