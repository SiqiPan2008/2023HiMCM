...
\section{Plant Impact Factor Model}
	...
	\subsection{Sensitivity Analysis}
		...
		We select 5 attributes for sensitivity analysis.  For each value of each attribute, we find the minimum and maximum global impact factor of the plants and plot them in \textbf{Fig.\ref{fig:IFfactors}}.  The minimum and maximum values all increases as the value of the attribute increases except the maximum for severe toxicity.  This may be due to a lack of data of severely toxic plants.
	...
	\subsection{Strengths and Weaknesses}
		...
		\begin{itemize}
			\vspace{-0.3cm}
			\item Our model is \textbf{easy to implement}.  We can design a questionnaire that enables us to calculate the impact factors quickly.
		\end{itemize}
		...
		\begin{itemize}
			\vspace{-0.3cm}
			\item Our model is \textbf{subjective}.  The weights are subjectively determined.  If experts had designated the weights, our model may be more accurate.
			
			\vspace{-0.3cm}
			\item Our attribute design may be \textbf{uncomprehensive}.  We chose the attributes to consider, so it is possible that we have overlooked some other important attributes.
		\end{itemize}
