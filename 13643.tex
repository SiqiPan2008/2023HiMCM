...
\section{Dandelion Spread Model}
	...
	\subsection{Dandelion Spread Results}
		...
		\subsubsection{Sensitivity Analysis}
			...
			\textbf{Fig.\ref{fig:saW}} shows the results for wind speed.  The number and mean distance have a positive correlation with $\mu_W$ and $\sigma_W$ and change drastically when the two factors vary.  When the mean wind speed increases, seeds are dispersed further away, so they are less likely to be
...

		{
			\fontsize{10}{14}\selectfont
			{
				\begin{longtable}{p{6.5in}}
					...
					\begin{algorithmic}
						\ForAll{\toB{$times$ } in \toB{$n$}}
						\ForAll{\toB{$days$ } in one year}
						\ForAll{\toB{$dandelion$ } in current all dandelions}
						\State \toB{$T$ } $\gets$ current temperature per \ref{}
						\EndFor
						\EndFor
						\EndFor
