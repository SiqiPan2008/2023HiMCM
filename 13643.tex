...
\begin{abstract}
	...
	The dandelion, officially named \textit{Taraxacum officinale}, is known as an invasive species, which is likely to cause economic or environmental harm or harm to human health.
	...
	We output all the locations of dandelions spreading on the land over time.  In order to get stable results, we run the model with 2 phases.  For each phase we run 1000 times.  In the first phase, we determine optimal starting time.  In the second phase, we fix the starting time and get the most probable outcome.
	...
	We apply our model to 226 invasive plants and calculate their global impact factors.  We also choose 4 plants and calculate their local impact factors at 3 locations.  Likewise, we conduct a sensitivity analysis and discuss the strengths and weaknesses.
	
	\vspace{0.5cm}
	At the end of this paper, we write an article to elaborate on our viewpoint regarding invasive species.  Rather than think they are enemies, we need to learn how to get along with them.  To us human beings, they are just vigorous friends.
...
\section{Introduction}
	...
	Dandelion is a very common plant with scientific name \textit{Taraxacum officinale}, which is stemless, lactiferous and perennial\autocite{stewart2002biology}.  We can find them at roadsides, in wild fields or at inconspicuous locations.  Just imagine that these little herbs with beautiful yellow flowers have been evolving on this planet for about 30 million years\autocite{dandelionhistory} - even longer than we human beings!
...
\section{Dandelion Spread Model}
	...
	\subsection{Assumptions and Justifications}
		...
		\begin{enumerate}
			...
			\begin{description}
				...
				Moreover, though wind rose maps provide the probabilities for each direction, without knowing the edge orientation of our spread area, we cannot decide the relative wind direction to the area.  So we assume wind direction is uniformly distributed from 0$^\circ$ to 360$^\circ$. In this way, all directions have same odds and we can pin the dandelion on an arbitrary edge.
			\end{description}
	...
	\label{sec:wind}
		...
		In the case of horizontal wind, due to large air drag on the pappus, we assume the pappus moves at a constant velocity in a statistical sense\autocite{tackenberg2003dandelion}. we have
	...
	\label{sec:af}
			...
			\begin{equation}
				...
				\end{gathered}
			\end{equation}
			The worst and best values for relevant parameters will be given in Section \ref{sec:life}.  If the a parameter exceeds the limits, it is set to the limit.
	...
	\label{sec:life}
		...
		\begin{itemize}
			...
			\item Out of all \textbf{seed}s, only 2\% will  germinate successfully\autocite{honek2005post}.  These seeds will spend 3 days growing into the developing stage\autocite{stewart2002biology}.
		...
		\begin{itemize}
			...
			\item When temperature $T > 24^\circ$C, only 12.5\% of the flowers bloom and disperse their seeds\autocite{yoshie2020effects}.  We let dandelions enter dispersal by probability 12.5\% and move the rest into the \textbf{hold} stage.  In this stage, flowers are held in buds.  As soon as $T$ falls below $24^\circ$C, flowers can 100\% bloom and disperse seeds.  
		
			\item Dandelions enter the \textbf{dormancy} stage when temperature $T < 0^\circ$C.  When the temperature is low, it is likely to snow and all life activities of dandelions are paused.  As soon as $T$ rises above $0^\circ$C, dandelions resume its previous stage.
	...
	\subsection{Algorithm}
		\vspace{-0.4cm}
		{
			\fontsize{10}{14}\selectfont
			{
				...
				\begin{algorithmic}
					\For{1 to \toB{$n$}}
					\ForAll{\toB{$t$ } in days of one year (360)}
					...
					\State \textbf{if} \toB{$T$ } < 0 \textbf{then} \toB{$previous\,stage$ } $\gets$ \toB{$current\,stage$}, \toB{$current\,stage$ } $\gets$ \textbf{Dormancy}
					\State \textbf{if} \toB{$current\,stage$ } is \textbf{Dormancy} and \toB{$T$ } > 0 \textbf{then} \toB{$current\,stage$ } $\gets$ \toB{$previous\,stage$}
					\State \textbf{if} \toB{$current\,stage$ } is \textbf{Hold} and \toB{$T$ } < 24 \textbf{then} \toB{$current\,stage$ } $\gets$ \toB{$previous\,stage$}
					
					\If{\toB{$current\,stage$ } is \textbf{Developing} or \toB{$current\,stage$ } is \textbf{Inter-dispersal}}
					\If{days of \toB{$current\,stage$ } reaches the next stage}
					\If{\toB{$T$ } > 24}
					\State \toB{$current\,stage$ } $\gets$ \textbf{Dispersal} with probability 0.125
					\If{\textbf{Dispersal} succeeds}
					\State \toB{$current\,stage$ } $\gets$ \textbf{Dispersal}
					\Else
					\State \toB{$previous\,stage$ } $\gets$ \toB{$current\,stage$}
					\State \toB{$current\,stage$ } $\gets$ \textbf{Hold}
					\EndIf
					\Else
					\State \toB{$current\,stage$ } $\gets$ \textbf{Dispersal}
					\EndIf
					...
					\If{\toB{$current\,stage$ } is \textbf{Seed}}
					\State \textbf{if} days of \toB{$current\,stage$ } reaches the next stage \textbf{then} \toB{$current\,stage$ } $\gets$ \textbf{Developing}
					\EndIf
					
					\If{\toB{$current\,stage$ } is \textbf{Dispersal}}
					\State determine the seed count that needs to be dispersed per Section \ref{sec:life}
					...
					\State generate a \toB{$new\,dandelion$ } with stage \textbf{Seed}
					\EndFor
					\State \textbf{if} days of \toB{$current\,stage$ } reaches the next stage \textbf{then} \toB{$current\,stage$ } $\gets$ \textbf{Inter-dispersal}
					\EndIf
					...
					\EndFor
					\EndFor
					\State calculate \toB{$numbers$ } and \toB{$mean\,distances$ } from \toB{$locations$}
					\State \textbf{Return}{ \toB{$locations$ }, \toB{$numbers$ } and \toB{$mean\,distances$}}
				\end{algorithmic}
...
\section{Plant Impact Factor Model}
	...
	\subsubsection{Local Impact Factor Results}
		...
		\vspace{-0.2cm}
		\begin{itemize}
		\item First, we choose 3 locations Alaska (AK), Florida (FL), and Nevada (NV), where Florida is warm and humid, Nevada is hot and dry, and Alaska is extremely cold;
		\vspace{-0.2cm}
		\item Then, we choose 4 plants that are invasive at these locations (the exceptions are: no trees of heaven in Alaska and no yellow salsifies in Nevada).  Sweetclover ranks the first in global impact factor; the tree of heaven is the highest ranking tree in terms of global impact factor (rank 3); yellow salsify has the highest global impact factor among nontoxic plants (rank 21); also, dandelion is not left out (rank 26).
		\vspace{-0.2cm}
	...
	\subsection{Strengths and Weaknesses}
		...
		\begin{itemize}
			\vspace{-0.3cm}
			\item Our model is \textbf{subjective}.  The weights are subjectively determined.  If we could involve more research results in determining the attribute weights, our model could be more accurate.
