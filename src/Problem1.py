...
for location, paras in envs.items():
    ...
    spread_results = spread_result_location[location]
    for ispread_result in range(len(spread_results)):
        spread_result = spread_results[ispread_result]
        output_results.append([ispread_result + 1, spread_result.number, spread_result.distance])
    u.output_csv('output\spread_runtimes_result_' + location + '.csv', output_results)

##########################################################################
# output spread course
##########################################################################
for location, paras in envs.items():
    row_time_course = [['month', 'number', 'mean_distance']]
    month_data = []
    ...
    for imonth in range(len(month_data)):
        ...
        u.output_csv('output\spread_course_' + str(imonth + 1) + '_' + location + '.csv', rows)

    u.output_csv('output\spread_month_result_' + location + '.csv', row_time_course)
